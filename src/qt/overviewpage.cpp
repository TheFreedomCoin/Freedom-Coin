...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeFreedomcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeFreedomcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeFreedomcoinAmount = strAnonymizeFreedomcoinAmount.remove(strAnonymizeFreedomcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFreedomcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeFreedomcoinAmount*COIN) nMaxToAnonymize = nAnonymizeFreedomcoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeFreedomcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeFreedomcoinAmount));
        strAnonymizeFreedomcoinAmount = strAnonymizeFreedomcoinAmount.remove(strAnonymizeFreedomcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeFreedomcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeFreedomcoinAmount == 0){
