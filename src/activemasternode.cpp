...
//
// Bootup the masternode, look for a 500 TFC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* donations are not supported in Freedomcoin.conf */
...
{
    CFreedomcoinAddress address(collateralAddress);
